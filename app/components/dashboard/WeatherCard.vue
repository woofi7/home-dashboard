      ...
      <div v-else class="bg-black/30 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden">
        <a :href="weather.url" target="_blank" rel="noopener" class="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors no-underline group">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="text-[11px] text-white/55 mt-0.5">{{ subLine }}</div>
          </div>
        ...
        <div v-if="display.laterToday && weather.laterToday?.length" class="border-t border-white/8 px-4 py-2">
        ...
        <div v-if="display.tomorrow && weather.tomorrow" class="border-t border-white/8 px-4 py-2.5 flex items-center gap-3">
